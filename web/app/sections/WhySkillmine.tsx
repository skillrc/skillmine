...
export default function WhySkillmine() {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-1/2 h-1/2 bg-brand-orange/5 rounded-full blur-[150px]" />
      ...
      <div className="container relative z-10">
        <div className="text-center mb-16">
          ...
          >
            Built for{' '}
            <span className="gradient-text">AI-native</span> workflows
          </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map((feature, idx) => (
            <div
              key={idx}
              className={`group p-6 rounded-2xl glass glass-border-hover transition-all duration-500 ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
              style={{ transitionDelay: isVisible ? `${300 + idx * 75}ms` : '0ms' }}
            >
              <div className="w-12 h-12 rounded-xl bg-brand-orange/10 border border-brand-orange/20 flex items-center justify-center mb-4 text-brand-orange transition-all duration-300 group-hover:bg-brand-orange group-hover:text-white group-hover:shadow-glow-orange group-hover:scale-110">
                {feature.icon}
              </div>
              <h3 className="text-lg font-semibold mb-2 text-text-primary group-hover:text-brand-orange transition-colors duration-300">
                {feature.title}
              </h3>
